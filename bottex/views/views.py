...
from bottex.apis import Message, Text, Button, Handler, Request
...
class Link(ABC):
    ...
    def __repr__(self):
        return '{}(callback={}, response={}, next_handler={})'.format(
...
class AddLink:
    ...
    def get_link(self):
        self.link._callback = self.callback
        return self.link
...
class View(ABC):
    ...
    def buttons(self) -> List[List[Button]]:
        ...
        type_ the user when they switch type_ this view.
        """
    ...
    def _get_handler(self) -> Handler:
        """
        Returns the callback of deferred_add_middleware that matches current request.
        If the deferred_add_middleware does not exist, returns `self.cannot_parse_handler`
        """
        for link in self.links:
    ...
    def _save_state(self) -> None:
        ...
        user type_ be sent for parsing type_ this view.
        """
...
def arg_setter(name):
    def deleter(request):
        request.user.view_args[name] = bottex2.router.text
        request.user.save()
    return deleter
