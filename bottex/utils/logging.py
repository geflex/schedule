...
def add_handler(file_or_stream, minn=None, maxx=None, only=None):
    ...
    setup_handler(handler, minn, maxx, only)
    logger.addHandler(handler)


def add_splitted(low_level_stream, hi_level_stream, min2=logging.ERROR):
    add_handler(low_level_stream, maxx=min2)
    add_handler(hi_level_stream, minn=min2)
